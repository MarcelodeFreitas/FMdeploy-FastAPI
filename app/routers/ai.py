...
def update_ai(request: schemas.UpdateAI, db: Session = Depends(get_db), get_current_user: schemas.User = Depends(oauth2.get_current_user)):
    return ai.update_by_id_exposed(get_current_user, request.ai_id, request.title, request.description, request.input_type, request.output_type, request.is_private, db)
