...
@router.get(
    "/is_admin", status_code=status.HTTP_200_OK, response_model=schemas.ShowUserAdmin
)
...
def get_all_users_for_admin(
    db: Session = Depends(get_db),
    get_current_user: schemas.User = Depends(oauth2.this_admin),
):
...
def create_administrator(
    ...
    return user.create_admin(
        get_current_user, request.name, request.email, request.password, db
    )
...
def update_current_user(
    ...
    return user.update_by_email(
        get_current_user, request.new_name, request.new_email, request.new_password, db
    )
